...
const CabelosStack = createStackNavigator();
...
const CATEGORIAS = [
  { nome: 'Shampoo',    icone: '🧴', tela: 'Shampoo'   },
    ...
    <View style={StyleSheet.absoluteFill} pointerEvents={visible ? 'auto' : 'none'}>
      ...
      <Animated.View style={[styles.drawerPanel, { transform: [{ translateX }] }]}>
        <View style={styles.drawerHeader}>
          <Text style={styles.drawerHeaderTitulo}>👩‍🦱 Cabelos</Text>
...
// ─── NAVIGATOR ────────────────────────────────────────────────────────────────
function CabelosNavigator() {
  return (
    // headerShown: false em TODOS — remove o header padrão do Stack
    <CabelosStack.Navigator screenOptions={{ headerShown: false }}>
        <CabelosStack.Screen name="CabelosPrincipal" component={CabelosPrincipal} />
        <CabelosStack.Screen name="Shampoo"         component={Shampoo}        />
       
    </CabelosStack.Navigator>
  );
}

// ─── TELA PRINCIPAL ───────────────────────────────────────────────────────────
function CabelosPrincipal({ navigation }) {
  const [drawerVisivel, setDrawerVisivel] = useState(false);
  return (
    <View style={{ flex: 1, backgroundColor: TEMA.fundo }}>
      <AppHeader
      ...
      <ScrollView contentContainerStyle={styles.scrollConteudo}>
        <View style={styles.banner}>
          <Text style={styles.bannerTitulo}>👩‍🦱 Cabelos</Text>
...
// ─── SHAMPOO ────────────────────────────────────────────────────────────────
const SHAMPOO = [
  { nome: 'Shampoo FreshClean',   preco: 'R$ 109,90',  img: require('./Cabelos/Shampoo/Fresh_Clean.jpg')  },
  { nome: 'Shampoo VitaHair',     preco: 'R$ 37,90',  img: require('./Cabelos/Shampoo/Vita_Hair.webp')    },
  { nome: 'Shampoo HydraPlus', preco: 'R$ 59,12',  img: require('./Cabelos/Shampoo/Hydra_Plus.webp')  },
  { nome: 'Shampoo PureCare',         preco: 'R$ 43,90',  img: require('./Cabelos/Shampoo/Pure_Care.webp') },
...
function Shampoo({ navigation }) {
  const { qtds, add, remove } = useProdutos(SHAMPOO);
  return (
    <TelaBase titulo="🧴 Shampoo" navigation={navigation}>
      {SHAMPOO.map((p, i) => (
...
export default CabelosNavigator;